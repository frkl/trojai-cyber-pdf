...
class new(nn.Module):
    def __init__(self,params):
        ...
        self.encoder=MLP(sum(self.chunks)*5,nh,2,nlayers);
    ...
    def forward(self,data_batch):
        ...
        for i in range(len(data_batch['fvs'])):
            hi=torch.Tensor(sum(self.chunks)*5).fill_(0);
            j=len(data_batch['fvs'][i])//5
            s=sum(self.chunks[:self.chunks.index(j)])
            hi[s*5:(s+j)*5]=data_batch['fvs'][i]
            h.append(hi)
